    ...
    */
    #pragma omp parallel sections num_threads(_threads_) private(in_block,hash,i)
    {
        ...
        */
        ...
        {
            uint64_t tmp = 0;
            for (i = 0; i < N / _threads_; i+=26)
            {
                write_first_pass(&in_block,   i+ 0); tmp += G_MD5(&in_block, &hash, i+ 0); // Perform MD5 sum
                write_pass(&in_block,   i+ 1); tmp += F_MD5(&in_block, &hash, i+ 1); // Perform MD5 sum
                write_pass(&in_block,   i+ 2); tmp += F_MD5(&in_block, &hash, i+ 2); // Perform MD5 sum
                write_pass(&in_block,   i+ 3); tmp += F_MD5(&in_block, &hash, i+ 3); // Perform MD5 sum
                write_pass(&in_block,   i+ 4); tmp += F_MD5(&in_block, &hash, i+ 4); // Perform MD5 sum
                write_pass(&in_block,   i+ 5); tmp += F_MD5(&in_block, &hash, i+ 5); // Perform MD5 sum
                write_pass(&in_block,   i+ 6); tmp += F_MD5(&in_block, &hash, i+ 6); // Perform MD5 sum
                write_pass(&in_block,   i+ 7); tmp += F_MD5(&in_block, &hash, i+ 7); // Perform MD5 sum
                write_pass(&in_block,   i+ 8); tmp += F_MD5(&in_block, &hash, i+ 8); // Perform MD5 sum
                write_pass(&in_block,   i+ 9); tmp += F_MD5(&in_block, &hash, i+ 9); // Perform MD5 sum

                write_pass(&in_block,   i+10); tmp += F_MD5(&in_block, &hash, i+10); // Perform MD5 sum
                write_pass(&in_block,   i+11); tmp += F_MD5(&in_block, &hash, i+11); // Perform MD5 sum
                write_pass(&in_block,   i+12); tmp += F_MD5(&in_block, &hash, i+12); // Perform MD5 sum
                write_pass(&in_block,   i+13); tmp += F_MD5(&in_block, &hash, i+13); // Perform MD5 sum
                write_pass(&in_block,   i+14); tmp += F_MD5(&in_block, &hash, i+14); // Perform MD5 sum
                write_pass(&in_block,   i+15); tmp += F_MD5(&in_block, &hash, i+15); // Perform MD5 sum
                write_pass(&in_block,   i+16); tmp += F_MD5(&in_block, &hash, i+16); // Perform MD5 sum
                write_pass(&in_block,   i+17); tmp += F_MD5(&in_block, &hash, i+17); // Perform MD5 sum
                write_pass(&in_block,   i+18); tmp += F_MD5(&in_block, &hash, i+18); // Perform MD5 sum
                write_pass(&in_block,   i+19); tmp += F_MD5(&in_block, &hash, i+19); // Perform MD5 sum

                write_pass(&in_block,   i+20); tmp += F_MD5(&in_block, &hash, i+20); // Perform MD5 sum
                write_pass(&in_block,   i+21); tmp += F_MD5(&in_block, &hash, i+21); // Perform MD5 sum
                write_pass(&in_block,   i+22); tmp += F_MD5(&in_block, &hash, i+22); // Perform MD5 sum
                write_pass(&in_block,   i+23); tmp += F_MD5(&in_block, &hash, i+23); // Perform MD5 sum
                write_pass(&in_block,   i+24); tmp += F_MD5(&in_block, &hash, i+24); // Perform MD5 sum
                write_pass(&in_block,   i+25); tmp += F_MD5(&in_block, &hash, i+25); // Perform MD5 sum
            }
            if (tmp != 0)
            {
        ...
        {
            uint64_t tmp = 0;
            for (i = 2 * N / _threads_; i < 3 * N / _threads_; i+=26)
            {
                write_first_pass(&in_block,   i+ 0); tmp += G_MD5(&in_block, &hash, i+ 0); // Perform MD5 sum
                write_pass(&in_block,   i+ 1); tmp += F_MD5(&in_block, &hash, i+ 1); // Perform MD5 sum
                write_pass(&in_block,   i+ 2); tmp += F_MD5(&in_block, &hash, i+ 2); // Perform MD5 sum
        ...
        {
            uint64_t tmp = 0;
            for (i = 3 * N / _threads_; i < 4 * N / _threads_; i+=26)
            {
                write_first_pass(&in_block,   i+ 0); tmp += G_MD5(&in_block, &hash, i+ 0); // Perform MD5 sum
                write_pass(&in_block,   i+ 1); tmp += F_MD5(&in_block, &hash, i+ 1); // Perform MD5 sum
                write_pass(&in_block,   i+ 2); tmp += F_MD5(&in_block, &hash, i+ 2); // Perform MD5 sum
        ...
        {
            uint64_t tmp = 0;
            for (i = 4 * N / _threads_; i < 5 * N / _threads_; i+=26)
            {
                write_first_pass(&in_block,   i+ 0); tmp += G_MD5(&in_block, &hash, i+ 0); // Perform MD5 sum
                write_pass(&in_block,   i+ 1); tmp += F_MD5(&in_block, &hash, i+ 1); // Perform MD5 sum
                write_pass(&in_block,   i+ 2); tmp += F_MD5(&in_block, &hash, i+ 2); // Perform MD5 sum
        ...
        {
            uint64_t tmp = 0;
            for (i = 6 * N / _threads_; i < 7 * N / _threads_; i+=26)
            {
                write_first_pass(&in_block,   i+ 0); tmp += G_MD5(&in_block, &hash, i+ 0); // Perform MD5 sum
                write_pass(&in_block,   i+ 1); tmp += F_MD5(&in_block, &hash, i+ 1); // Perform MD5 sum
                write_pass(&in_block,   i+ 2); tmp += F_MD5(&in_block, &hash, i+ 2); // Perform MD5 sum
        ...
        {
            uint64_t tmp = 0;
            for (i = 8 * N / _threads_; i < 9 * N / _threads_; i+=26)
            {
                write_first_pass(&in_block,   i+ 0); tmp += G_MD5(&in_block, &hash, i+ 0); // Perform MD5 sum
                write_pass(&in_block,   i+ 1); tmp += F_MD5(&in_block, &hash, i+ 1); // Perform MD5 sum
                write_pass(&in_block,   i+ 2); tmp += F_MD5(&in_block, &hash, i+ 2); // Perform MD5 sum
        ...
        {
            uint64_t tmp = 0;
            for (i = 10 * N / _threads_; i < 11 * N / _threads_; i+=26)
            {
                write_first_pass(&in_block,   i+ 0); tmp += G_MD5(&in_block, &hash, i+ 0); // Perform MD5 sum
                write_pass(&in_block,   i+ 1); tmp += F_MD5(&in_block, &hash, i+ 1); // Perform MD5 sum
                write_pass(&in_block,   i+ 2); tmp += F_MD5(&in_block, &hash, i+ 2); // Perform MD5 sum
        ...
        {
            uint64_t tmp = 0;
            for (i = 11 * N / _threads_; i < 12 * N / _threads_; i+=26)
            {
                write_first_pass(&in_block,   i+ 0); tmp += G_MD5(&in_block, &hash, i+ 0); // Perform MD5 sum
                write_pass(&in_block,   i+ 1); tmp += F_MD5(&in_block, &hash, i+ 1); // Perform MD5 sum
                write_pass(&in_block,   i+ 2); tmp += F_MD5(&in_block, &hash, i+ 2); // Perform MD5 sum
        ...
        {
            uint64_t tmp = 0;
            for (i = 13 * N / _threads_; i < 14 * N / _threads_; i+=26)
            {
                write_first_pass(&in_block,   i+ 0); tmp += G_MD5(&in_block, &hash, i+ 0); // Perform MD5 sum
                write_pass(&in_block,   i+ 1); tmp += F_MD5(&in_block, &hash, i+ 1); // Perform MD5 sum
                write_pass(&in_block,   i+ 2); tmp += F_MD5(&in_block, &hash, i+ 2); // Perform MD5 sum
        ...
        {
            uint64_t tmp = 0;
            for (i = 14 * N / _threads_; i < 15 * N / _threads_; i+=26)
            {
                write_first_pass(&in_block,   i+ 0); tmp += G_MD5(&in_block, &hash, i+ 0); // Perform MD5 sum
                write_pass(&in_block,   i+ 1); tmp += F_MD5(&in_block, &hash, i+ 1); // Perform MD5 sum
                write_pass(&in_block,   i+ 2); tmp += F_MD5(&in_block, &hash, i+ 2); // Perform MD5 sum
...
void write_first_pass(union Block *in_block, uint64_t i)
{
    in_block->_8[6] = alph(i % E1);
    i /= E1;
    in_block->_8[5] = alph(i % E1);
...
uint64_t G_MD5(union Block *bl, union Hash *ha, uint64_t i)
{
    ...
    */

    {
        ...
        FF1(bl->_32[3], b, c, d, bl->_32[ 0], S11, 0xd76aa478); /* 1 */
        a=bl->_32[3];
        FF (d, a, b, c, bl->_32[ 1], S12, 0xe8c7b756); /* 2 */
#ifndef LEN7
        FF (c, d, a, b, bl->_32[ 2], S13, 0x242070db); /* 3 */
